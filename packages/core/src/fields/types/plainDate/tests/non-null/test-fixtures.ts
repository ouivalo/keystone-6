...
export const name = 'plainDate with isNullable: false';
export const typeFunction = (x: any) => plainDate({ ...x, db: { ...x?.db, isNullable: false } });
...
export const getTestFields = () => ({ lastOnline: plainDate({ db: { isNullable: false } }) });
