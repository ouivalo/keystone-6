...
It accepts an object with list names as keys, and `list()` or `singleton()` configurations as values.
...
This document will explain the configuration options which can be used with the `list()` and `singleton()` functions.

## List Types - Standard Lists and Singletons

Keystone has two types of lists, the first is a standard list that uses the `list()` function and the second
type is a [singleton](<https://en.wikipedia.org/wiki/Singleton_(mathematics)>) list that uses the `singleton()` function, a list that uses the `singleton()` function will only support one entry.
The standard list will be the best choice in most situations.
...
(ie filtering, `findMany`, `create`) for more details on singletons see [our Singleton guide](../guides/singletons).
...
- `cacheHint` (default: `undefined`): Allows you to specify the [dynamic cache control hints](https://www.apollographql.com/docs/apollo-server/performance/caching/#in-your-resolvers-dynamic) used for queries to this this list.
